({
	 getData : function(cmp) {
        var action = cmp.get('c.getJeffOppoList');
        action.setCallback(this, $A.getCallback(function(response) {
            var state = response.getState();
            console.log(response.getReturnValue());
            if (state === "SUCCESS") {
                var oppos = response.getReturnValue().oppos;
                oppos.forEach(element => {
                    element.spaceUrl = "/one/one.app#/sObject/" + element.Space__c;
                    element.spaceName = element.Space__r.Name;
                    element.oppoUrl = "/one/one.app#/sObject/" + element.Id;
                });
                    //debugger
   ...
   saveChanges: function (cmp, draftValues) {
       var self = this;
       var action = cmp.get('c.updateOppos');
